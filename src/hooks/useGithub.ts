...
export function normalizeUsername(value: string): string {
  ...
  if (!/^[a-z\d](?:[a-z\d-]{0,37}[a-z\d])?$/i.test(username) || username.includes('--')) {
    throw new Error('Enter a GitHub username, not a profile URL (1–39 letters, numbers, or single hyphens).');
  }
  return username;
}

async function githubRequest(url: string, signal?: AbortSignal): Promise<Response> {
  ...
  if (response.status === 403 || response.status === 429) {
    ...
    throw new Error(`GitHub request limit reached. Try again ${when}, or explore the demo meanwhile.`);
  }
  throw new Error(`GitHub is unavailable (${response.status}). Please try again.`);
}

export async function fetchProfile(value: string, signal?: AbortSignal): Promise<ProfileResult> {
  const username = normalizeUsername(value);
  const userResponse = await githubRequest(`https://api.github.com/users/${encodeURIComponent(username)}`, signal);
  ...
  for (let page = 1; page <= 5; page++) {
    try {
      const response = await githubRequest(`https://api.github.com/users/${encodeURIComponent(username)}/repos?per_page=100&sort=updated&page=${page}`, signal);
      const batch: GithubRepo[] = await response.json();
      repos.push(...batch);
      if (batch.length < 100) break;
      if (page === 5 && user.public_repos > 500) warning = 'Showing the 500 most recently updated public repositories. Older repositories are not included.';
    } catch (error) {
  ...
  return { user, repos: repos.sort((a, b) => b.stargazers_count - a.stargazers_count), warning };
}
